...
experiment=experiment; %Mogelijkheden: sp (maar 1 crossover en 1 mutation), ap
%(alle parameters), cw (beste crossover weg), mw (beste mutator weg)
seed=num2str(seed);
nofCities=num2str(nofCities);
nofmetainds=num2str(nofmetainds);
nofmaxmetagens=num2str(nofmaxmetagens);
alpha=num2str(alpha);
expstring=strcat('resultaten/',experiment,'s',seed,'c',nofCities,'n',nofmetainds,'g',nofmaxmetagens,'a',alpha);
end